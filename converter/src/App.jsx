...
function App() {
  ...
  useEffect(() => {
     if(parseInt(fromAmount) === 0){
      ...
      fetch(`https://api.frankfurter.app/latest?amount=${fromAmount}&from=${fromCurrency}&to=${toCurrency}`).then(res => res.json().then(data => setToAmount(Object.values(data.rates))))
     }
